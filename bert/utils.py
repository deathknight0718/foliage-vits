...
from bert.commons import init_weights, get_padding, fused_add_tanh_sigmoid_multiply
from bert.transforms import piecewise_rational_quadratic_transform
...
class MelStyleEncoder(nn.Module):
    ...
    def __init__(self, n_mel_channels=80, style_hidden=128, style_vector_dim=256, style_kernel_size=5, style_head=2, dropout=0.1):
        ...
        self.spectral = nn.Sequential(LinearNorm(self.in_dim, self.hidden_dim), Mish(), nn.Dropout(self.dropout), LinearNorm(self.hidden_dim, self.hidden_dim), Mish(), nn.Dropout(self.dropout))
        ...
        self.slf_attn = MultiHeadAttention(self.n_head, self.hidden_dim, self.hidden_dim // self.n_head, self.hidden_dim // self.n_head, self.dropout)
